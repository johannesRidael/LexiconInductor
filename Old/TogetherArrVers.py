...
def conFinder(sents, oq, dicti, cons, spac, iternum, restr):
    #print('proc helper')
    connsCands = Counter()
    for doc in sents:
        for sent in doc.sents:
            ...
            for k in range(len(nps)):
                if nps[k] in dicti or searchFromBack(restr, nps[k]):
                    if k < len(nps) - 1 and (nps[k + 1] in dicti or searchFromBack(restr, nps[k+1])):
                        connect = sent[snps[k].end:snps[k+1].start].text
                        if connect not in cons:
...
def wordFinder(sents, oq, connectors, dicti, spac, iternum, rstr):
    poss = Counter()
    for doc in sents:
        for sent in doc.sents:
            pT = sent
...
if __name__ == '__main__':
    ...
    print('creating sentence array')
    sentArr = []
    docArr = []
    for dirr in os.scandir(r'D:\eeeeee\Downloads\Spring2022\IE\curatedCorps\using'):
        for doc in DocBin().from_disk(dirr.path).get_docs(spac.vocab):
            docArr.append(doc)
            for sent in doc.sents:
                sentArr.append(sent)
    print('finished, beginning induction')
    sentPerProc = int((len(sentArr)+1) / SIMULPROCCOUNT)
    docPerProc = int((len(docArr)+1)/SIMULPROCCOUNT)
    wcandidates = Counter()
    ccandidates = Counter()
    while len(dic) < 1000:
        ...
        for i in range(SIMULPROCCOUNT):
            p = mp.Process(target=conFinder, args=(docArr[i*docPerProc:(i+1)*docPerProc], cQue, dic, cons, spac, it, reStr))#sentArr[i*sentPerProc:(i+1)*sentPerProc]
            p.start()
            procs.append(p)
            it += 1
        ...
        it = 0
        for i in range(SIMULPROCCOUNT):
            p = mp.Process(target=wordFinder, args=(docArr[i*docPerProc:(i+1)*docPerProc], cQue, cons, dic, spac, it, reStr))
            p.start()
            procs.append(p)
            it += 1
