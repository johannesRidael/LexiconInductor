...
def searchFromBack(reStr, searchString):
    ...
    if len(split) > 0:
        ...
        if match != None and match.end() >= len(split[len(split) - 1]) - 2 and match.start() < 2:
            return True
    return False
...
if __name__ == '__main__':
    ...
    while len(dic) < 1000:
        ...
        mst = findTop5(npPoss, acandidates, adj)
        ms = [m[1] for m in mst]
        for m in ms:
            acandidates.pop(m)
        adj.update(ms)
        ...
        mswc = findTop5(npPoss, wcandidates, dic)
        ms = [m[1] for m in mswc]
        for m in ms:
            wcandidates.pop(m)
        lastW = ms
        for mc in mswc:
            s = mc[1].split()
            if len(s) > 0:
                print(mc[1], 'added to the dictionary with score ', mc[0])
                dic.add(mc[1])
                reStr += '|' + s[len(s) - 1]
