...
if __name__ == '__main__':
    ...
    procs = []
    try:
        npPoss = probables.CountMinSketch(filepath='CMSdemo.cms')
    except:
        ...
        npPoss.export('CMSdemo.cms')

    print('please enter 5 seed words')
    ...
    print('please type which Exclusionary Categories you would like to use: ')
    print('1: Birds, 2: Locations, 3: Fishing Terms, 4: Non-fish Underwater Animals, 5: Biological Terms, 6: Fish')
    uIn = input().strip()
    print('beginning search. please standby')

    altSeeds = ['birds', 'locations', 'fishing', 'weirdUnd', 'biology', 'seeds']
    altDics = []
    for i in range(len(uIn)):
        altDics.append(set())
        for line in open('seeds/' + altSeeds[int(uIn[i]) - 1] + '.txt'):
            excl.add(line.strip())
    ...
    while len(dic) < 1000:
        ...
        for m in ms:
            s = m[1].split()
            if len(s) > 0:
                print(m[1], 'added with score: ', m[0])
                dic.add(m[1])
                reStr += '|' + s[len(s) - 1]
        ...
        for i in range(len(altDics)):
            ...
            out = open('demoAlts.txt', 'a', encoding='utf-8')
            for m in ms:
                s = m[1].split()
                if len(s) > 0:
                    altDics[i].add(m[1])
